...
export default class RecentSongDatabase {
  async Add(user_id: string, song_id: string) {
    var sql = "INSERT INTO recentsong(user_id, Id) VALUES (?,?)";
    var check = await Mysql2.query(sql, [user_id, song_id]);
    return check;
  }
  ...
  async GetRecentSong(song_id: string, user_id: string) {
    var sql = `SELECT song.* FROM recentsong, song WHERE recentsong.user_id=? AND recentsong.Id=? AND song.Id = recentsong.Id `;
    var check = (await Mysql2.query(sql, [user_id, song_id])) as [];
    return check;
  }
  async Get(user_id: string, Id_song: string) {
    var sql = `SELECT recentsong.* FROM recentsong WHERE recentsong.user_id=? AND recentsong.Id=?`;
    var check = await Mysql2.query(sql, [user_id, Id_song]);
    return check;
  }
  async UpdateTime(user_id: string, song_id: string) {
    var sql = `UPDATE recentsong SET Time=CURRENT_TIMESTAMP,personal_View=personal_View+1 WHERE user_id=? AND Id=?`;
    var check = await Mysql2.query(sql, [user_id, song_id]);
    return check;
  }
  async ListenAgainByUserId(user_id: string, start: number) {
    var sql = `SELECT song.*,likedsong.liked
                ...
                ORDER BY recentsong.personal_View ASC
                LIMIT ?,50`;
