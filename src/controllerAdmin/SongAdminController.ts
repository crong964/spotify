...
class SongAdminController {
  ...
  async Add(req: Request, res: Response) {
    ...
    if (req.file != undefined) {
      try {
        song.SongImage = (await firebase.UploadImageBuffer(
          `SongImage/${song.Id}`,
          req.file.buffer,
          20000
        )) as string;
    ...
    let lsPlayListArtist = await Promise.all(
      singer.map(async (v) => {
  ...
  async Upload(req: Request, res: Response) {
    ...
    if (req.body.name == undefined) {
      ...
      var fcheck = await Promise.all([
        SongAdminController.song.Add(song),
    ...
    var write = createWriteStream(
      path.join(process.cwd(), "/public/music", `${f}`),
      {
        flags: "as+",
      }
    );
  ...
  async DeleteSong(req: Request, res: Response) {
    ...
    var check = await Promise.all([
      SongAdminController.song.Delete(idsong),
    ...
    //    `SongImage/${data[1].Id}`,
    //    `delete/SongImage/${data[1].Id}_${d}`
    //  );
    //}

    if (check) {
      try {
        let idfolder = await googleDrive.SearchNameFolder(
          data[1]?.filePath || ""
        );
