...
export class PlayListController {
    ...
    if (req.file) {
      try {
        playlistmodel.ImagePath =
          (await PlayListController.firebase.UploadImageBufferNoZip(
  ...
  async PlayListDetailAdmin(req: Request, res: Response) {
    ...
    let ls = await Promise.all([
      PlayListController.playlist.Get(idplaylist),
      PlayListController.contain.GetAllByPlayList(idplaylist),
      PlayListController.genre.GetIdParentByIdplaylist(idplaylist),
    ]);
    res.json({
      err: ls[0] == undefined,
      playlist: ls[0],
  ...
  async UpdatePlayListDetailAdmin(req: Request, res: Response) {
    ...
    if (req.file) {
      try {
        await PlayListController.firebase.DeleteFile(
          `playlist/${playlistmodel.id}`
        );
        //await PlayListController.firebase.MoveImage(
        //  `playlist/${playlistmodel.id}`,
        //  `deletePlaylist/playlist/${playlistmodel.id}`
        //);
        playlistmodel.ImagePath =
          (await PlayListController.firebase.UploadImageBufferNoZip(
            `playlist/${playlistmodel.id}`,
  ...
  async GetPlayListById(req: Request, res: Response) {
    ...
    let ls = await Promise.all([
      PlayListController.playlist.Get(id_playlist),
  ...
  async NextPlayListLimit(req: Request, res: Response) {
    ...
    let ls = await Promise.all([
      PlayListController.playlist.GetPlayListLimit(start, count),
  ...
  async NextPlayArtistListLimit(req: Request, res: Response) {
    ...
    let ls = await Promise.all([
      PlayListController.playlist.GetPlayListArtistLimit(start, count, id),
  ...
  async Delete(req: Request, res: Response) {
    ...
    if (playlist && playlist.User_id == id) {
      await Promise.all([
        PlayListController.contain.DeleteAll(idplaylist),
      ...
      if (playlist?.ImagePath.length > 0) {
        //PlayListController.firebase.Move(
        //  `playlist/${playlist?.id}.webp`,
        //  `delete/playlist/${playlist?.id}.webp`
        //);

        PlayListController.firebase.DeleteFile(`playlist/${playlist?.id}`);
      }
    }
    res.json({
  ...
  async Update(req: Request, res: Response) {
    ...
    let playlistmodel = await PlayListController.playlist.Get(req.body.id);

    if (!playlistmodel || playlistmodel.User_id != id) {
      return res.json({
        err: false,
      });
    }
    ...
    if (req.file) {
      try {
        ...
        let check = await PlayListController.processImage.ConvertImgToWebp(
          path,
          `${path}.webp`
        );
        if (check) {
