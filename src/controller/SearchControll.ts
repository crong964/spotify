...
class SearchControll {
  ...
  async SearchSongNameWithoutPlaylist(req: Request, res: Response) {
    ...
    let ls = await SearchControll.song.SearchSongNameWithoutPlaylist(
      name,
      idPlaylist,
      status
    );
  ...
  async SearchNameArtistAndSong(req: Request, res: Response) {
    ...
    var ls = await Promise.all([
      SearchControll.likedSong.SearchName(name, id),
  ...
  async SearchName(req: Request, res: Response) {
    ...
    var id = req.cookies.id;
    var ls = await Promise.all([
      SearchControll.haveListFriends.SearchName(name, id, "2"),
