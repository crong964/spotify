...
class GoogleDrive {
  ...
  DownloadStreamFile(id: string) {
    return GoogleDrive.service.files.get(
      { fileId: id || "12", alt: "media" },
      { responseType: "stream" }
    );
  }
  ...
  async SearchNameFile(name: string): Promise<string[]> {
    return new Promise((res, rej) => {
      GoogleDrive.service.files
        .list({ q: `name contains '${name}'` })
        .then((v) => {
          let ls: string[] = [];
  ...
  CreateFoder(
    name: string
  ): Promise<{ id: string | undefined | null; err: string | undefined }> {
    return new Promise((res, rej) => {
      GoogleDrive.service.files
        .create({
          requestBody: {
