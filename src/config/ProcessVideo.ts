...
const mp4fragment = production
  ? join(process.cwd(), "/dist/tool/mp4fragment")
  : join(process.cwd(), "/src/tool/mp4fragment.exe");
const mp4split = production
  ? join(process.cwd(), "/dist/tool/mp4split")
  : join(process.cwd(), "/src/tool/mp4split.exe");
...
class ProcessVideo {
  ...
  async Mp4Fragment(input: string, out: string) {
    let s = await this.Command(
      `${mp4fragment} --fragment-duration 5000 ${input} ${out}`
    );
    console.log(s.ok);
    return s;
  }
  ...
  async Mp4Split(input: string, out: string) {
    ...
    let s = await this.Command(
      `${mp4split} --init-segment ${out}.init --audio --media-segment ${out}-%llu.%llu.m4s ${input}`
    );
    console.log(s.ok);
