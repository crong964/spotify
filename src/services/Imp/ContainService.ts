...
export class ContainService {
  ...
  async DeleteC(Song_id: string, PlayList_id: string, User_id: string) {
    ...
    var check = (await Mysql2.query(sql, [
      Song_id,
      PlayList_id,
      User_id,
    ])) as ResultSetHeader;
  ...
  async GetAllByPlayList(PlayList_id: string) {
    var sql =
      "SELECT song.Id,song.SongName,song.Viewer,song.Singer,song.Duration,song.filePath,song.SongImage, contain.TimeCreate FROM contain,song WHERE contain.Song_ID=song.Id AND contain.PlayList_id=?";
    var check;
    check = await Mysql2.query(sql, [PlayList_id]);
...
const containServiceImp = new ContainService();
