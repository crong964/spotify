...
export class GenreService {
  ...
  async Add(genre: GenreModel) {
    ...
    var sql =
      " INSERT INTO genre(Id, Name, RightGenre, LeftGenre,idParent,Floor) VALUES (?,?,?,?,?,?)";
    var check;
  ...
  async CreateBlank(Right: number) {
    var sql =
      "UPDATE genre SET RightGenre = RightGenre + 2 WHERE RightGenre >= ? ";
    var sql2 = "UPDATE genre SET LeftGenre = LeftGenre + 2 WHERE LeftGenre > ?";
    var check = await Promise.all([
      Mysql2.query(sql, [Right]),
      Mysql2.query(sql2, [Right]),
    ]);
    return check;
  }
  ...
  async DeleteBlank(id: string) {
    ...
    check = await Promise.all([
      Mysql2.query(sql, [genre.RightGenre + ""]),
      Mysql2.query(sql2, [genre.RightGenre + ""]),
    ]);
  ...
  async GetChildrenByIdParent(idParent: string) {
    var sql =
      "SELECT g2.* FROM genre g1,genre g2 WHERE g1.id =? AND g1.RightGenre > g2.RightGenre AND g1.LeftGenre < g2.LeftGenre ";
    var check;
    check = await Mysql2.query(sql, [idParent]);
    return this.Setls(check);
  }
