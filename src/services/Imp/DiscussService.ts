...
export class DiscussService {
  ...
  async Add(d: DiscussModel) {
    var sql =
      "INSERT INTO discuss(User_Id, Discuss_Id, Parent_discuss_Id, Replay_Discuss_Id, Content,Song_Id,Type) VALUES (?,?,?,?,?,?,?)";
    var check;
  ...
  async GetMainDiscussBySong_Id(Song_Id: string) {
    var sql =
      "SELECT discuss.*,user.pathImage, user.Name FROM discuss,user WHERE discuss.User_Id=user.id AND discuss.Song_Id= ? AND discuss.Type=0 ORDER BY discuss.createtime DESC";
    var check;
  ...
  async GetReplayDiscussByParentDiscussId(Parent_discuss_Id: string) {
    var sql =
      "SELECT discuss.*,user.pathImage, user.Name FROM discuss,user WHERE discuss.User_Id=user.id AND discuss.Parent_discuss_Id=?";
    var check;
  ...
  async Increase(Parent_discuss_Id: string, n?: number) {
    n = n || 1;
    var sql =
      "UPDATE discuss SET Replay_quality = Replay_quality + ? WHERE Discuss_Id= ?";
    var check;
