...
class PlayListLikeService {
  ...
  async Get(user_id: string, playlist_id: string) {
    let sql =
      "SELECT playlist.* FROM playlistlikes, playlist WHERE playlistlikes.User_ID =? AND playlistlikes.PlayList_id=? AND playlist.id = playlistlikes.PlayList_id";
    let check = (await Mysql2.query(sql, [
      user_id,
      playlist_id,
    ])) as RowDataPacket[];
  ...
  async GetAll(user_id: string) {
    let sql =
      "SELECT playlist.* FROM playlistlikes, playlist WHERE playlistlikes.User_ID =? AND playlist.id = playlistlikes.PlayList_id ORDER BY playlistlikes.data DESC";
    let check = await Mysql2.query(sql, [user_id]);
    return this.Setls(check);
  }
  async UpdateDate(user_id: string, PlayList_id: string) {
    let sql =
      "UPDATE playlistlikes SET `data`=CURRENT_TIMESTAMP() WHERE PlayList_id =? AND User_ID=? ";
