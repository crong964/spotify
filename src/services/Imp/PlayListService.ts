...
export class PlayListService {
  ...
  async AddArtists(d: PlayListModel) {
    var sql =
      "INSERT INTO playlist(id, User_id, Genre_ID, Type, ImagePath, PlayListName, Likes, Songs, Duration, Status, Discripition) VALUES (?,?,?,'artist',?,?,?,?,?,?,?)";
    var check;
    check = await Mysql2.query(sql, [
      d.id,
      d.User_id,
  ...
  async Update(d: PlayListModel) {
    var sql =
      "UPDATE playlist SET ImagePath=?,PlayListName=?,Likes=?,Songs=?,Duration=?,Status=?,Discripition=? WHERE id =?";
    var check;
    check = await Mysql2.query(sql, [
      d.ImagePath,
      d.PlayListName,
  ...
  async UpdateNameImage(d: PlayListModel) {
    var sql =
      "UPDATE playlist SET ImagePath=?,PlayListName=?,Discripition=? WHERE id =? AND User_id=?";
    var check;
    check = await Mysql2.query(sql, [
      d.ImagePath,
      d.PlayListName,
...
var playListServiceImp: PlayListService = new PlayListService(
